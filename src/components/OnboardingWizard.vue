...
// ── Step: welcome | path | search | create | players | payment | done
...
// ── Player invite via email ──────────────────────────────────────────────────
async function doSendEmail() {
  const email = pEmail.value.trim()
  if (!email) { inviteErr.value = 'Enter an email address to send an invite'; return }
  inviteErr.value = ''
  inviting.value = true
  try {
    ...
    const resp = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-invite-email`,
      {
        ...
        body: JSON.stringify({
          invitee_email: email,
          invitee_name: pName.value.trim() || email.split('@')[0],
    ...
    if (!playerList.value.find(p => p.email === email)) {
      playerList.value.push({
        name:  pName.value.trim() || email.split('@')[0],
        email,
        token,
      })
    }
...
function doWhatsApp() {
  ...
  if (latestToken.value) {
    const url = `https://badminton360.app/join?token=${latestToken.value}`
    msg = name
      ? `Hey ${name}! 🏸 Join ${newClubName.value} on Badminton 360 — we track rankings, split costs & run tournaments.\nAccept your invite: ${url}`
      : `🏸 You're invited to join ${newClubName.value} on Badminton 360!\nAccept here: ${url}`
  } else {
    const club = newClubName.value || 'our club'
    msg = `Hey${name ? ' ' + name : ''}! 🏸 We're using Badminton 360 for ${club} — Elo rankings, expense splits & tournaments.\nSign in at https://badminton360.app and search for "${club}" to join!`
  }
  window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`, '_blank')
}

// ── Save player to local list and reset form ─────────────────────────────────
function commitPlayer() {
  const name  = pName.value.trim()
  const email = pEmail.value.trim()
  if (!name && !email) return
  if (!playerList.value.find(p => p.email && p.email === email)) {
    playerList.value.push({
      ...
      >
        ...
        <div v-if="step === 'welcome'" class="flex flex-col overflow-y-auto">
          ...
          <div class="px-6 pb-8 pt-4 flex flex-col gap-3">
            ...
            <button
              @click="finish()"
              class="text-xs text-slate-400 hover:text-slate-600 transition py-1"
            >
        ...
        <div v-if="step === 'path'" class="flex flex-col overflow-y-auto">
          ...
          <div class="px-6 pt-6 pb-4">
            <button
              @click="step = 'welcome'"
              class="text-sm text-slate-400 hover:text-slate-600 transition flex items-center gap-1 mb-5"
            >← Back</button>
        ...
        <div v-if="step === 'search'" class="flex flex-col overflow-y-auto">
          ...
          <div class="px-6 flex flex-col gap-3 overflow-y-auto flex-1 pb-4">
            ...
            <div v-if="!searching" class="flex flex-col gap-2">
              ...
              <p
                v-if="searchQ.trim() && !searchResults.length"
        ...
        <div v-if="step === 'create'" class="flex flex-col overflow-y-auto">
          ...
          <div class="px-6 pt-6 pb-4">
            <button
              ...
              class="text-sm text-slate-400 hover:text-slate-600 transition flex items-center gap-1 mb-5"
            >← Back</button>
            <div class="text-[10px] uppercase tracking-widest text-slate-400 font-semibold mb-4">
              Step 1 of 3
            </div>
        ...
        <div v-if="step === 'players'" class="flex flex-col overflow-y-auto">
          ...
          <div class="px-6 pt-6 pb-3 shrink-0">
            ...
            <p class="text-sm text-slate-500">Add players one by one. Send invites by email or WhatsApp.</p>
          </div>

          <!-- Invited list -->
          <div v-if="playerList.length" class="px-6 pb-2 shrink-0">
          ...
          <!-- Form (scrollable) -->
          <div class="px-6 pt-2 pb-3 flex flex-col gap-3 overflow-y-auto flex-1">
            <div>
            ...
            <div class="flex gap-2 pt-1">
              <button
                @click="doSendEmail"
                :disabled="inviting"
                class="flex-1 py-3 rounded-xl text-sm font-bold text-white transition-all hover:opacity-90 active:scale-[.99] disabled:opacity-60"
                :style="emailSent
                  ? 'background:#dcfce7;color:#166534;'
                  : 'background:linear-gradient(135deg,#00b4d8,#0077a8);'"
              >
                {{ inviting ? 'Sending…' : emailSent ? '✓ Email Sent!' : '📧 Send Email Invite' }}
              </button>
              <button
                @click="doWhatsApp"
                class="flex-1 py-3 rounded-xl text-sm font-bold transition-all hover:opacity-90 active:scale-[.99]"
          ...
          <div class="px-6 pb-6 shrink-0 border-t pt-4" style="border-color:rgba(0,0,0,.07);">
            <div class="flex gap-2 mb-2">
              <button
                @click="doSaveAndAddAnother"
                class="flex-1 py-3 rounded-xl text-sm font-semibold border transition hover:bg-slate-50 active:scale-[.99]"
                style="border-color:rgba(0,0,0,.12); color:#475569;"
              >
                + Save &amp; Add Another
              </button>
              <button
                @click="doSavePlayer"
                class="flex-1 py-3 rounded-xl text-sm font-bold text-white transition hover:opacity-90 active:scale-[.99]"
            ...
            <button
              @click="step = 'payment'"
              class="w-full text-xs text-slate-400 hover:text-slate-600 transition py-1"
            >
        ...
        <div v-if="step === 'payment'" class="flex flex-col overflow-y-auto">
              ...
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500 mt-1 leading-relaxed">
                    Players contribute upfront. Auto-deduct per session. No more chasing payments.
                  </div>
                </div>
              </div>
            </button>
            ...
            >
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500 mt-1 leading-relaxed">
                    Record expenses and split equally. Track who owes what at a glance.
                  </div>
                </div>
              </div>
            </button>
            ...
            >
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div class="text-xs text-slate-500 mt-1 leading-relaxed">
                    Set opening balances to carry over amounts from your previous system.
                  </div>
                </div>
              </div>
            </button>
          ...
          <div class="px-6 pb-6 shrink-0">
            <button
              @click="paymentChoice('/dashboard')"
              class="w-full text-sm text-slate-400 hover:text-slate-600 transition py-2"
            >
        ...
        <div v-if="step === 'done'" class="flex flex-col items-center text-center overflow-y-auto">
          ...
          <div class="flex-1 px-8 pt-10 pb-4 w-full">
            ...
            <p class="text-slate-500 text-sm leading-relaxed mb-8">
              Your club is ready to go. Record your first match and let the rankings begin!
            </p>
            ...
            <div class="text-left space-y-2.5 mb-6">
              <div class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium" style="background:#f0fdf4; color:#166534;">
                <span>✅</span>
                <span>Club created — you're the <strong>owner</strong></span>
              </div>
              <div v-if="playerList.length" class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium" style="background:#f0fdf4; color:#166534;">
                <span>✅</span>
                <span>{{ playerList.length }} player{{ playerList.length !== 1 ? 's' : '' }} invited</span>
              </div>
              <div class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium" style="background:#f0fdf4; color:#166534;">
                <span>✅</span>
                <span>Elo rankings start at 1000 — fair for everyone</span>
              </div>
            </div>
          </div>
