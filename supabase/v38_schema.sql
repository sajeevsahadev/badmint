...
-- Suggests a balanced 2v2 doubles lineup from the day's attendees.
...
BEGIN
  ...
  -- 1. Collect attendees for the day
  SELECT array_agg(a.player_id)
  INTO v_attendee_ids
  FROM attendance a
  WHERE a.club_id = p_club_id AND a.played_on = p_date;

  IF v_attendee_ids IS NULL OR array_length(v_attendee_ids, 1) < 4 THEN
    RETURN jsonb_build_object(
      'error', 'Need at least 4 players marked as attending for this date'
    );
  END IF;

  -- 2. Who played in the MOST RECENT match today? (deprioritize them for rotation)
  ...
  -- 3. Build sorted candidate list:
  --    Primary sort: players NOT in last match first (rotation)
  --    Secondary sort: Elo DESC (for snake-draft balancing)
  SELECT jsonb_agg(
    jsonb_build_object(
      'id',   p.id,
  ...
  --    This minimises Elo gap between teams.
...
GRANT EXECUTE ON FUNCTION suggest_lineup(uuid, date) TO authenticated;
